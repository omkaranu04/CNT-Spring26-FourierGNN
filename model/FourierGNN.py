...
class FGN(nn.Module):
    def __init__(self, pre_length, embed_size, feature_size, seq_length, hidden_size, hard_threholding_fraction=1, hidden_size_factor=1, sparsity_threshold=0.01):
        super().__init__()
        ...
        self.hard_threholding_fraction = hard_threholding_fraction
    ...
    def fourierGC(self, x, B, N, L):
        o1_real = torch.zeros([B, (N * L) // 2 + 1, self.frequency_size * self.hidden_size_factor], device=x.device)
        o1_imag = torch.zeros([B, (N * L) // 2 + 1, self.frequency_size * self.hidden_size_factor], device=x.device)
        ...
        o1_real = F.relu(torch.einsum('bli, ii->bli', x.real, self.w1[0]) - torch.einsum('bli, ii->bli', x.imag, self.w1[1]) + self.b1[0])
        o1_imag = F.relu(torch.einsum('bli, ii->bli', x.real, self.w1[1]) + torch.einsum('bli, ii->bli', x.imag, self.w1[0]) + self.b1[1])
        ...
        o2_imag = F.relu(torch.einsum('bli,ii->bli', o1_real, self.w2[1]) + torch.einsum('bli,ii->bli', o1_imag, self.w2[0]) + self.b2[1])
